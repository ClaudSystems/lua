...
class ListFaturaViewModel {
    ...
    private String id
    Item pickedItem
    private ListModelList<Fatura> selectedItems
    ...
    private List<Item> getAllItems() {
        ...
        for (Fatura x:faturas){
            if (!x.equals(null)){
                items.add(new Item(x.id,x.cliente.nome, x.valorTotal, x.dateCreated,x.numeroDaFatura,x.estado))
            }
    ...
    class Item {
        ...
        Item(Long id,String nome, BigDecimal valorTotal, Date dateCreated,String numeroDaFatura,String estado) {
            this.id = id
        ...
        if (filter == null || "".equals(filter)) {
            ...
            for (Item item : allItems) {
                if (item.getNomeCliente().toLowerCase().indexOf(filter.toLowerCase()) >= 0
                        ...
                        item.getValorTotal().toString().indexOf(filter) >= 0 ||
    ...
    def findItem(){
        ...
        if(itemDB.equals(null)){
          //  Messagebox.show("Não foi localizado a Fatura com o ID= "+id, "Lua", 1,  Messagebox.ERROR)
            info.value = "Não foi localizado a Fatura com o ID= "+id
        }else {
