...
private renderFieldForProperty(p, owningClass, prefix = "") {
 ...
 <div class="form-group">
   	<div class="col-lg-12">
		<div class=" col-lg-3 \${hasErrors(bean: ${propertyName}, field: '${prefix}${p.name}', 'error')} ${required ? 'required' : ''}">
			<label for="${prefix}${p.name}" class="col-lg-3"><g:message code="${domainClass.propertyName}.${prefix}${p.name}.label" default="${p.naturalName}" /><% if (required) { %><span class="required-indicator">*</span><% } %></label>
		</div>
		<div class="col-lg-9">
			${renderEditor(p)}
